<script lang="ts">
	...
	function handleOpenChange() {
		...
		if ($isOpen) {
			boundingBoxStyle = '';
			fetch(serverBaseURL + '/species_detail?id=' + species?.id)
				.then((response) => {
					if (!response.ok) {
				...
				.then((data) => {
					...
					let setBoundingBoxStyle = function () {
						if (
							newSpeciesData.x !== undefined &&
							newSpeciesData.y !== undefined &&
							newSpeciesData.width !== undefined &&
							newSpeciesData.height !== undefined
						) {
							boundingBoxStyle = calculateBoundingBoxStyle(
								newSpeciesData.x,
								newSpeciesData.y,
								newSpeciesData.width,
								newSpeciesData.height
							);
						}
					};
							...
							{#if species.taxonomy != null}
								<div class="innerTaxonomyContainer">
									{#each species.taxonomy?.ancestors as ancestor, index}
										<div class="taxonomyItem" style={'margin-left:' + index * 12 + 'px'}>
											<b>{ancestor.name}</b> ({ancestor.rank})
										</div>
									{/each}
									<div
										class="taxonomyItem"
										style={'margin-left:' + species.taxonomy?.ancestors.length * 12 + 'px'}
									>
										<b>{species.concept}</b> ({species.rank})
									</div>
									{#each species.taxonomy?.descendants as descendant, index}
										<div
											class="taxonomyItem"
											style={'margin-left:' +
												(species.taxonomy?.ancestors.length + index) * 12 +
												'px'}
										>
											<b>{descendant.name}</b> ({descendant.rank})
										</div>
									{/each}
								</div>
							{/if}
